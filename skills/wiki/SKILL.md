...
## Extract Source Images into Raw Records

When the original material contains substantive images—figures, diagrams, charts, screenshots, infographics, table images, or X Article media—extract and archive them during ingest by default. A remote image URL or an empty placeholder alone is not a complete raw capture.

1. Inventory image references in the source before drafting: Markdown images, HTML `img`/`srcset`/lazy-load attributes, X post/Article media, EPUB image members, and figures embedded in PDFs. Exclude tracking pixels, decorative UI icons, unrelated recommendation thumbnails, and avatars unless the source uses them as evidence.
2. Download or extract the original image bytes in source order under `_wiki/raw/assets/<raw-stem>/`, using stable names such as `001-figure.png` or `002-screenshot.jpg`. Keep a PDF/EPUB's original binary separately when it is the canonical source asset. Never put cookies, tokens, browser data, or signed session URLs in the Vault.
3. Replace the source image marker at the same point in the raw body with a vault-local Obsidian embed, for example `![[_wiki/raw/assets/<raw-stem>/001-figure.png|原始 alt 或 caption]]`. Keep the original alt text, caption, figure number, and nearby explanation adjacent to the embed; do not move all images to an appendix.
4. Keep the raw Markdown body clean and image-focused: insert only the vault-local image embed plus source-provided alt text, caption, and nearby explanation. Never add or display inline HTML provenance comments, and do not expose the image source URL beside the image. Store per-image provenance privately in `_wiki/raw/assets/<raw-stem>/_provenance.json` (source URL, source location, local path, extraction status, media type, and byte SHA-256); the canonical source URL may also remain in raw frontmatter.
5. Verify every archived file is non-empty and decodes as an image, every local embed resolves, and the sidecar provenance matches the archived bytes. Reuse an image when the same URL or byte hash repeats within one source, while preserving each occurrence's position. Do not silently deduplicate across sources unless each source's provenance remains explicit in its sidecar.
6. If retrieval or decoding fails, keep a visible source-location marker such as `[图片未成功归档：原因]` without printing the original image URL in the raw body; record the safe URL and failure details in the sidecar or private validation output, and mark the raw capture `partial`/lower its confidence as appropriate. Never replace a missing figure with a generated image or an unverified third-party copy.
7. Recompute the raw body SHA-256 only after local image embeds, failure markers, and the final Mermaid appendix are complete. Run `audit_vault.py`, a sidecar/provenance check, and a local-embed existence check before reporting success.
